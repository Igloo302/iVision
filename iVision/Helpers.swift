...
func nonMaxSuppression(boxes: [YOLO.Prediction], limit: Int, threshold: Float) -> [YOLO.Prediction] {
  ...
  // 算法很简单：从得分最高的方框开始。删除所有与之重叠超过给定阈值量的剩余框。如果还有剩余的框（即这些框与先前的任何框都不重叠），请重复此过程，直到没有更多的框或达到限制为止。
...
/**
  Computes intersection-over-union overlap between two bounding boxes. 算一算两个框之间的重叠度
*/
